...
if __name__ == "__main__":
    ...
    with tqdm(total=5 if args.mini_data else len(dataset)) as pbar:
        for i, datum in enumerate(dataset):
            ...
            if args.task == "add":
                mix_label = datum["source_audio"]["category"]
                mix_label.extend(datum["reference_audio"]["category"])
                edit_label = datum["edit"]["event"]
                mix_label = set(mix_label)
                edit_label = set(edit_label)
                mix_label = [list(mix_label - edit_label)]
                edit_label = [list(edit_label)]
                prompt_src = label2caption(mix_label)[0]
                prompt_tgt = label2caption(mix_label, background_sound=edit_label)[0]
                ...
                edit_label = datum["edit"]["event"]
                mix_label = set(mix_label)
                mix_label = [list(mix_label - set(edit_label))]
                edit_label = [edit_label]
                prompt_tgt = label2caption(mix_label)[0]
                ...
                prompt_src = label2caption(mix_label, background_sound=edit_label_src)[
                    0
                ]
                prompt_tgt = label2caption(mix_label, background_sound=edit_label_tgt)[
                    0
                ]

            out_audio = run_ddim_inversion(
                file_path=datum["source_audio"]["audio_path"],
